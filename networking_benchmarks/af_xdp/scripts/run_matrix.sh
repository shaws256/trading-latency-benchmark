...
echo "--- Starting packet_replicator on all nodes ---"
for i in $(seq 0 $((FLEET_SIZE - 1))); do
  echo "  [${i}] Starting replicator on ${NODE_NAMES[$i]}..."
  # Kill any existing replicator, then start fresh in background
  ssh_cmd "${PUBLIC_IPS[$i]}" "sudo pkill -f packet_replicator 2>/dev/null || true; sleep 1; \
    cd ${REMOTE_BIN} && sudo nohup ./packet_replicator eth0 ${PRIVATE_IPS[$i]} ${PORT} true --queues ${REPLICATOR_QUEUES} \
...
for i in $(seq 0 $((FLEET_SIZE - 1))); do
  ssh_cmd "${PUBLIC_IPS[$i]}" "sudo pkill -f packet_replicator 2>/dev/null || true" &
done
wait
