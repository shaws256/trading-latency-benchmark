...
export class FleetStack extends cdk.Stack {
  constructor(scope: cdk.App, id: string, props: FleetStackProps) {
    ...
    const getOrCreatePG = (strategy: PlacementStrategy, az: string): CfnPlacementGroup => {
      const key = strategy === 'cluster' ? `cluster:${az}` : strategy;
      if (!placementGroups.has(key)) {
        const pgId = strategy === 'cluster'
          ? `PG-cluster-${az.replace(/[^a-zA-Z0-9]/g, '')}`
          : `PG-${strategy}`;
    ...
    // ── AMI + Key ────────────────────────────────────────────────────────
    // ── AMI resolution (priority: explicit amiId > latest AL2023) ──────────
    // SSM auto-resolution available when param /af-xdp/ami/<region> exists
    // (written by ami-builder). Pass --context amiId=<id> to use explicitly.
    ...
    const fleetManifest: {
      ...
      az: string; region: string; placement: string | null; outputPrefix: string;
    }[] = [];
    ...
    for (const entry of primaryEntries) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (placement) {
          const pg = getOrCreatePG(placement, az);
          (inst.node.defaultChild as ec2.CfnInstance).placementGroupName = pg.ref;
          Tags.of(inst).add('PlacementStrategy', placement);
        }
        ...
        fleetManifest.push({
          ...
          region: primaryRegion, placement, outputPrefix: prefix,
        });
        globalIndex++;
      }
    }
    ...
    if (secondaryRegion && secondaryEntries.length > 0) {
      ...
      for (const entry of secondaryEntries) {
        ...
        for (let i = 0; i < count; i++) {
          ...
          fleetManifest.push({
            index: globalIndex, instanceType: instType, role, az,
            region: secondaryRegion, placement, outputPrefix: prefix,
          });
          globalIndex++;
        }
      }
  ...
  private validateEntries(entries: (FleetEntry & { _resolvedAz: string })[]): void {
    // Cluster: all must share one AZ
    const clusterAZs = new Set<string>();
    ...
    for (const entry of entries) {
      ...
      if (entry.placement === 'spread') {
        spreadPerAz.set(az, (spreadPerAz.get(az) ?? 0) + count);
      }
    }

    if (clusterAZs.size > 1) {
      throw new Error(
        `Cluster placement requires all instances in the same AZ. ` +
        `Got: ${Array.from(clusterAZs).join(', ')}`
      );
    }
