...
export class ControlPlaneStack extends cdk.Stack {
  constructor(scope: cdk.App, id: string, props: ControlPlaneStackProps) {
    ...
    ud.addCommands(
      ...
      '( cd "$CP/web" && npm ci && npm run build && cp -r dist /opt/af-xdp-cp/web-dist ) || echo "web build skipped"',
      'NATSVER=v2.10.22',
      ...
      'WEB=""; [ -d /opt/af-xdp-cp/web-dist ] && WEB="-web /opt/af-xdp-cp/web-dist"',
      'cat > /etc/systemd/system/cp-backend.service <<UNIT\n[Unit]\nDescription=AF_XDP control-plane backend\nAfter=cp-nats.service\nWants=cp-nats.service\n[Service]\nExecStart=/opt/af-xdp-cp/afxdp-backend -nats ${SCHEME}://127.0.0.1:4222 -nats-token ${NATS_TOKEN} ${BE_TLS} -addr :8080 ${WEB}\nRestart=always\nRestartSec=3\n[Install]\nWantedBy=multi-user.target\nUNIT',
