...
export class ControlPlaneStack extends cdk.Stack {
  constructor(scope: cdk.App, id: string, props: ControlPlaneStackProps) {
    ...
    ud.addCommands(
      ...
      'cat > /etc/systemd/system/cp-backend.service <<UNIT\n[Unit]\nDescription=AF_XDP control-plane backend\nAfter=cp-nats.service\nWants=cp-nats.service\n[Service]\nExecStartPre=/bin/mkdir -p /var/lib/af-xdp-cp\nExecStart=/opt/af-xdp-cp/afxdp-backend -nats ${SCHEME}://127.0.0.1:4222 -nats-token ${NATS_TOKEN} ${BE_TLS} -addr :8080 -web /opt/af-xdp-cp/web-dist -db-path /var/lib/af-xdp-cp/measurements.db -retention-days 7\nRestart=always\nRestartSec=3\n[Install]\nWantedBy=multi-user.target\nUNIT',
